...
class _NotificationScreenState extends State<NotificationScreen> with LoadingView {
  bool _isLoad = false;
  ...
  void initState() {
    Future.delayed(Duration.zero).then((value) async {
      ...
      Provider.of<NotificationProvider>(context, listen: false).fnGetNotification();
    });
    super.initState();
  }
        ...
        builder: (context, provider, _) {
          return TabBar(
            ...
            tabs: [
              Tab(
                child: Badge(
                  badgeContent: Text(provider.infoCount.toString(), style: TextStyle(
                    color: Colors.white,
                    fontSize: 12,
                  ),),
                  badgeColor: CustomColor.BROWN_TXT,
                  padding: EdgeInsets.all(6),
                  position: BadgePosition.topEnd(top: -15, end: -18),
                  child: Text('Info'),
                ),
              ),
              Tab(
                child: Badge(
                  badgeContent: Text(provider.promoCount.toString(), style: TextStyle(
                    color: Colors.white,
                    fontSize: 12,
                  ),),
                  badgeColor: CustomColor.BROWN_TXT,
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Container(
                              ...
                              child: InkWell(
                                onTap: () async {
                                  if (_data.type == 'info') {
                                    await provider.fnOnReadNotification(_data.id ?? 0);
                                    await Get.toNamed(TransactionDetailScreen.tag, arguments: TransactionDetailScreen(
                                      orderId: _data.codeTransaction,
                                      regionId: _data.regionId,
                                    ))!.then((_) async => await provider.fnGetNotification());
                                  }
                                },
                                child: Padding(
                                      ...
                                      Flexible(
                                        child: ListTile(
                                          ...
                                          title: Column(
                                            ...
                                            children: [
                                              Row(
                                                ...
                                                children: [
                                                  Text(_data.type ?? '-', style: TextStyle(
                                                    color: CustomColor.GREY_TXT,
                                                  ),),
                                                  _data.isNew == 1
                                                      ? Icon(Icons.circle, color: CustomColor.MAIN, size: 10,)
                                                      : Container(),
                                                ],
                                              ),
                                              SizedBox(height: 5,),
                                          ...
                                          subtitle: Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: [
                                              Text(_data.body ?? '-', style: TextStyle(
                                                color: Colors.black,
                                              ),),
                                              SizedBox(height: 10,),
                                              Row(
                                                ...
                                                children: [
                                                  Text("${timeago.format(_data.createdAt != null
                                                      ? DateTime.parse(_data.createdAt!)
                                                      : DateTime.now(), locale: 'en_short')} ago",
                                                    style: TextStyle(
                                                      color: CustomColor.GREY_TXT,
                                                    ),
                                                  ),
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: CustomColor.BG,
          centerTitle: true,
          title: Text(AppLocalizations.instance.text('TXT_NOTIFICATION'), style: TextStyle(
